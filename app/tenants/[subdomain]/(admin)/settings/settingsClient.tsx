...
export default function SettingsClient({ subdomain, tenantId }: SettingsClientProps) {
    ...
    return (
        <div className="mx-auto space-y-6 text-foreground select-none animate-in fade-in duration-500">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input value={profile.initials}
                                       ...
                                       className="bg-background border-border h-11 text-center font-bold text-sm rounded-md" />
                            </div>
            ...
            {/* 🚀 LIVE: PLANS SECTION COMPONENTIZED */}
            {currentView === "PLANS" && (
                ...
                <div className="space-y-6">
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            ...
                            <RuleInputBox title="Check-in Window" desc="Minutes before session start to allow ingress access"
                                          val={rules.checkInWindow} unit="min"
                                          onChange={(v) => setRules({ ...rules, checkInWindow: v })} />
                            <RuleInputBox title="Default Session" desc="Standard block duration for assets or human resources"
                                          val={rules.defaultSession} unit="min"
                                          onChange={(v) => setRules({ ...rules, defaultSession: v })} />
                            <RuleInputBox title="Cancellation Notice" desc="Minimum notice required to drop a slot penalty-free"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <Input type="number" value={taxGateway.vatPercentage}
                                           onChange={(e) => setTaxGateway({ ...taxGateway, vatPercentage: Number(e.target.value) })}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input type="number" value={taxGateway.ssclPercentage}
                                           onChange={(e) => setTaxGateway({ ...taxGateway, ssclPercentage: Number(e.target.value) })}
                                           className="bg-background border-border h-10 text-sm rounded-md font-mono" />
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input type="password" value={taxGateway.payhereSecret}
                                           onChange={(e) => setTaxGateway({ ...taxGateway, payhereSecret: e.target.value })}
                                           className="bg-background border-border h-10 text-sm rounded-md font-mono" />
                                </div>
                            </div>
                        </div>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6 overflow-hidden">
                        <div className="divide-y divide-border">
                            <ToggleRow title="Session reminder" desc="Notify member 24 hrs before their session booking starts"
                                       checked={notifications.sessionReminder} onChange={(v) => setNotifications({ ...notifications, sessionReminder: v })} />
                            <ToggleRow title="Payment due reminder" desc="Alert member via automated SMS 7 days before membership token expiration"
                                       checked={notifications.paymentDue} onChange={(v) => setNotifications({ ...notifications, paymentDue: v })} />
                            <ToggleRow title="Low token alert" desc="Message member automatically when available consumable assets drop below fallback levels"
                                       checked={notifications.lowToken} onChange={(v) => setNotifications({ ...notifications, lowToken: v })} />
                            <ToggleRow title="Welcome message" desc="Send automated welcome payload packet to new members upon core identity linkage"
                                       checked={notifications.welcomeMessage} onChange={(v) => setNotifications({ ...notifications, welcomeMessage: v })} />
                            <ToggleRow title="Session log confirmation" desc="Notify member immediately after a coach updates personal session tracking statistics"
                                       checked={notifications.sessionLog} onChange={(v) => setNotifications({ ...notifications, sessionLog: v })} />
                            <ToggleRow title="Check-in alert to trainer" desc="Ping coach accounts immediately when an assigned athlete handles hardware ingress validation"
                                       checked={notifications.checkInAlert} onChange={(v) => setNotifications({ ...notifications, checkInAlert: v })} />
                        </div>
            ...
            {currentView === "ACCESS" && (
                <div className="space-y-6">
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6">
                        <div className="divide-y divide-border">
                            <ToggleRow title="Member self check-in" desc="Allow consumer accounts to execute entry transactions via personal QR/IoT parameters without front-desk manual override steps"
                                       checked={accessControl.selfCheckIn} onChange={(v) => setAccessControl({ ...accessControl, selfCheckIn: v })} />
                            <ToggleRow title="Trainer confirms session logs" desc="Forced validation boundary: session metric logs are locked until assigned trainer profile approves data states"
                                       checked={accessControl.trainerConfirms} onChange={(v) => setAccessControl({ ...accessControl, trainerConfirms: v })} />
                            <ToggleRow title="Member-initiated cancellations" desc="Permit end-consumers to drop resource reservations directly inside threshold parameters via the B2C portal app layer"
                                       checked={accessControl.memberCancellations} onChange={(v) => setAccessControl({ ...accessControl, memberCancellations: v })} />
                            <ToggleRow title="Members see class roster" desc="Expose community profiles: allow linked users to view fellow attendees checked into a current scheduling block"
                                       checked={accessControl.seeClassRoster} onChange={(v) => setAccessControl({ ...accessControl, seeClassRoster: v })} />
                            <ToggleRow title="Trainers see revenue data" desc="Platform visibility override: allow horizontal visibility of raw tenant revenue metrics and financial dashboards on standard trainer screens"
                                       checked={accessControl.seeRevenue} onChange={(v) => setAccessControl({ ...accessControl, seeRevenue: v })} />
                        </div>
...
// --- Dynamic Visual Component Extensions (Now exported for child components) ---
