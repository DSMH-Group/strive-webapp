...
export default function SettingsClient({ subdomain, tenantConfig }: SettingsClientProps) {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 text-foreground select-none animate-in fade-in duration-500">
            ...
            {currentView === "MENU" && (
                <div className="space-y-6">
                    ...
                    <div className="bg-card/40 border border-border rounded-lg overflow-hidden divide-y divide-border">
                        ...
                        <MenuListItem icon={<CreditCard className="text-muted-foreground" />} title="Membership Plans" desc="Pricing and session token allocation" onClick={() => setCurrentView("PLANS")} />
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-2 space-y-2">
                                ...
                                <Input value={profile.name} onChange={(e) => setProfile({...profile, name: e.target.value})} className="bg-background border-border h-11 text-sm rounded-md focus-visible:ring-primary/20" />
                            </div>
                            <div className="space-y-2">
                                <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Tagline</Label>
                                <Input value={profile.tagline} onChange={(e) => setProfile({...profile, tagline: e.target.value})} className="bg-background border-border h-11 text-sm rounded-md focus-visible:ring-primary/20" />
                            </div>
                        </div>
                        ...
                        <Table>
                            <TableHeader className="hover:bg-transparent border-b border-border">
                                <TableRow className="border-b border-border hover:bg-transparent">
                                    <TableHead className="text-xs text-muted-foreground font-bold tracking-wider pl-0">PLAN</TableHead>
                                    <TableHead className="text-xs text-muted-foreground font-bold tracking-wider">MONTHLY PRICE (LKR)</TableHead>
                                    <TableHead className="text-xs text-muted-foreground font-bold tracking-wider">SESSION TOKENS</TableHead>
                                    <TableHead className="text-xs text-muted-foreground font-bold tracking-wider text-right pr-0">ALLOCATION CAP</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {plans.map((plan, idx) => (
                                    <TableRow key={plan.id} className="border-b border-border hover:bg-transparent">
                                        <TableCell className="pl-0 font-bold text-foreground text-sm py-4">{plan.name}</TableCell>
                                        <TableCell>
                                            <Input
                                                type="number"
                                                value={plan.price}
                                                ...
                                                className="w-36 bg-background border-border font-mono h-9 text-sm rounded-sm"
                                            />
                                        </TableCell>
                                        <TableCell>
                                            <Input
                                        ...
                                        <TableCell className="text-right pr-0 text-xs font-bold text-muted-foreground font-mono">
                                            <span className={cn(
                                                "px-2.5 py-1 rounded-full border border-border bg-background/60",
            ...
            {/* 4. LOCAL TAXATION & PAYMENT GATEWAY MODIFIERS */}
