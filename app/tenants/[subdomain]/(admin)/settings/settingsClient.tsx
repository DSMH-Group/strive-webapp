...
export default function SettingsClient({subdomain, tenantId}: SettingsClientProps) {
    ...
    const {data: fetchedPlans = [], isLoading: plansLoading} = useQuery({
        ...
        enabled: currentView === "PLANS" // Only fetch if the user opens this view!
    });
    ...
    const savePlansMutation = useMutation({
        mutationFn: async (plansToProcess: any[]) => {
            ...
            const promises = plansToProcess.map(plan => {
                ...
                if (plan.isDirty && !plan.isDeleted) {
    ...
    const updateLocalPlan = (idx: number, field: string, value: any) => {
        const updated = [...localPlans];
        updated[idx] = {...updated[idx], [field]: value, isDirty: true};
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="md:col-span-1 space-y-2">
                                <Label
                                    className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Phone</Label>
                            ...
                            <div className="md:col-span-2 space-y-2">
                                <Label
                                    className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Email</Label>
                        ...
                        <div className="space-y-2">
                            <Label
                                className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Address</Label>
            ...
            {currentView === "PLANS" && (
                <div className="space-y-6">
                    ...
                    <Card className="bg-card/30 border-border rounded-lg p-6">
                        {plansLoading ? (
                            <div className="py-12 flex justify-center"><Loader2
                                className="w-6 h-6 animate-spin text-muted-foreground"/></div>
                        ) : (
                            <Table>
                                <TableHeader className="hover:bg-transparent border-b border-border">
                                    <TableRow className="border-b border-border hover:bg-transparent">
                                        <TableHead
                                            className="text-xs text-muted-foreground font-bold tracking-wider pl-0 w-1/3">PLAN
                                            NAME</TableHead>
                                        <TableHead className="text-xs text-muted-foreground font-bold tracking-wider">PRICE
                                            (LKR)</TableHead>
                                        <TableHead
                                            className="text-xs text-muted-foreground font-bold tracking-wider">TOKENS</TableHead>
                                        <TableHead
                                            className="text-xs text-muted-foreground font-bold tracking-wider text-right pr-0">ACTION</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {localPlans.filter(p => !p.isDeleted).map((plan, idx) => (
                                        <TableRow key={plan.id}
                                                  className="border-b border-border hover:bg-transparent group">
                                            <TableCell className="pl-0 py-3">
                                                <Input
                                                    type="text"
                                                    value={plan.name}
                                                    onChange={(e) => updateLocalPlan(idx, 'name', e.target.value)}
                                            ...
                                            <TableCell className="py-3">
                                                <Input
                                                    ...
                                                    onChange={(e) => updateLocalPlan(idx, 'monthlyPrice', Number(e.target.value))}
                                            ...
                                            <TableCell className="py-3">
                                                <Input
                                                    ...
                                                    onChange={(e) => updateLocalPlan(idx, 'sessionTokens', Number(e.target.value))}
                                            ...
                                            <TableCell className="text-right pr-0 py-3">
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={() => updateLocalPlan(idx, 'isDeleted', true)}
...
function MenuListItem({icon, title, desc, onClick}: {
...
function RuleInputBox({title, desc, val, unit, onChange}: {
    title: string;
    desc: string;
    val: number;
    unit: string;
